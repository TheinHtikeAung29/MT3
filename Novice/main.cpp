...
float LengthSq(const Vector3& v) { return v.x * v.x + v.y * v.y + v.z * v.z; }

Vector3 Scale(const Vector3& v, float s) { return {v.x * s, v.y * s, v.z * s}; }

Vector3 Project(const Vector3& v1, const Vector3& v2) {
	float d = Dot(v1, v2);
	float lenSq = LengthSq(v2);
	return Scale(v2, d / lenSq);
}

Vector3 ClosestPoint(const Vector3& point, const Segment& segment) {
	Vector3 toPoint = Subtract(point, segment.origin);
	float t = Dot(toPoint, segment.diff) / LengthSq(segment.diff);
	if (t < 0.0f)
		t = 0.0f;
	if (t > 1.0f)
		t = 1.0f;
